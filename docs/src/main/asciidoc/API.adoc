...
* +ConversionContext+ provides additional metdata, inclusing the key accessed, which is useful in many use cases.
...
public interface PropertyValueCombinationPolicy{
   ...
   String collect(Map<String,String> currentValue currentValue, String key, PropertySource propertySource);
