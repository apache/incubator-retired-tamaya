...
* Both, sections as well as parameter models
  ** can be modelled by a met-data entry, by default +_{key}.model+.
...
* Parameters additionally have
  ** a _type_, described by the fully qualified class name, into which any configured (String) value must be
     convertable into. If no type is configured +java.lang.String+ is assumed as default.
...
_config-model.provider=ConfigModel Extension

# org.mycompany.root (optional section)
_org.mycompany.root.model-type=Section
...
_org.mycompany.security.model-type=Section
...
# minmal section
_minimal.model-class=Section

# custom validated section
_validated.model-type=Section
...
_org.mycompany.root.name.model-type=Parameter
...
# A minmal parameter
_minimalClass.model-type=Class
-------------------------------------------------------------------------------
...
The +Usage+ class itself provides access to further fainer grained usage data (+AccessDetail+) cotaining:
...
The methods for managing and tracking of configuration changes are delegated to the current
most sinificant implementation of the +org.apache.tamaya.model.spi.ConfigUsageStatsSpi+ SPI.
